...
SECURITY_CHECKS = ("Dangerous-Workflow", "Token-Permissions", "Binary-Artifacts",
...
def advisories(ecosystem, name, version):
    ...
    for v in d.get("vulns", []):
        if v.get("withdrawn"):
            continue
        sev = max((s.get("type", "") and _cvss_band(s) for s in v.get("severity", [])),
                  key=lambda x: ("", "low", "medium", "high", "critical").index(x or ""), default=None)
        out.append({"id": v["id"], "summary": (v.get("summary") or "")[:120],
                    "severity": sev or _db_severity(v)})
    return out
...
def verdict(ev):
    ...
    if sev & {"critical", "high"}:
        ids = [a["id"] for a in ev["advisories"] if a.get("severity") in ("critical", "high")]
        reject.append(f"unpatched advisory in current version: {', '.join(ids[:3])}")
    elif ev.get("advisories"):
        concern.append(f"{len(ev['advisories'])} lower-severity advisor"
                       f"{'y' if len(ev['advisories']) == 1 else 'ies'}")
...
def security_state(ev):
    ...
    if ev.get("advisories") is None and not ev.get("scorecard_date"):
        return "not assessed"
...
def probe(slug):
    ...
    if sc:
        ...
        ev["failed_security_checks"] = [
            (c["name"], c["score"]) for c in sc.get("checks", [])
            if c["name"] in SECURITY_CHECKS and 0 <= c.get("score", -1) < 5
        ]
    else:
...
def selftest():
    ...
    assert _cvss_band({"score": "CVSS:3.1/AV:N/9.8"}) == "critical"
    assert _cvss_band({"score": "5.3"}) == "medium"
    assert _cvss_band({"score": "n/a"}) is None
